...
Finally, to implement Google auth, you will need the `GoogleOAuthProvider` to wrap your application. The following command will install it, plus Apple Auth sign in, which will be used later:
...
Once your `main.tsx` is ready, lets create the login buttons, that will look like this
...
Then, include a `.env` file in the root of your project, adding it to `.gitignore`, then updating the file with the following values from the Sequence Builder:
...
- massive gas savings for proejcts that require multiple tokens as a single ERC1155 can hold many different varieties.
...
And to allow the Transaction API to relay transactions without a fee, sponsor gas by following [this guide](/solutions/builder/gas-tank#watch-the-gas-tank-in-action-in-builder) for the deployed contract.

## 5. Deploy A Cloudflare Worker

Once we have our token contract deployed, we will also deploy a serverless worker that handles token mints using the Sequence Transaction API for us in a secure & scalable way. 
...
In the template, we've included code to call an already publicly available, deployed API which randomly generates diablo-like loot along with the code to parse the response.
...
Then the `getInferenceWithItem` function is called where we pass in a `prompt` which is the generated loot `name` and `type` returned from the diablo API, as well as some additional parameters.

<Callout type='info'>
We used the `EulerDiscreteScheduler` parameter for the scheduler type which is a good balance between quality and image generation time, however if you want to experiment with different scheduler parameters, you can use [this custom local cli](https://github.com/moskalyk/scenario-gg-benchmark-cli) and check the results in the [Scenario.gg](https://scenario.gg/) dashboard.
</Callout>
...
Then once we have the `inferenceId` we can poll the inference status and return when complete. This way we can notify the user on the client-side about the status of the image generation process which can take a few moments:
...
Complete and integrate [this guide](/guides/metadata-guide) in order to learn about the Sequence Metadata API.

Then once complete, pass the stored media url to the frontend:

```typescript
	...
	const response = await uploadAsset(...) // Example function taken from Sequence Metadata guide
