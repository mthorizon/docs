...
Then complete the `getInferenceWithItem` function and passing in a `prompt` which is basically the generated loot `name` and `type`, as well as some additional parameters
...
Generating the loot from the various functions like `generate` and calling the method like so to return an `inferenceId`
...
and call the `getInferenceObjectWithPolling` with the `inferenceId` passed in, where when you receive the response, you can obtain the image url with `resObject.inference.images[0].url`
...
You should now understand how to mint AI art for your Scenario.gg and Sequence powered game. 
