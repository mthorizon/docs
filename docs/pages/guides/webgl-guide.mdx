...
1. [Project Setup With Webpack](#1-project-setup-with-webpack): Enable a project structure with WebGL to be compiled by Webpack
2. [Integrate Sequence Kit](#2-integrate-sequence-kit): Allow all EOAs and Sequence Wallet to authenticate the user
3. [Deploy a Collectibles Contract](#3-deploy-a-collectibles-contract): Create your own collectible contract
4. [Deploy a Remote Minter and Mint In-game Tokens](#4-deploy-a-remote-minter--mint-in-game-achievement-tokens): Perform gasless relayed transactions with Cloudflare workers
5. [Leverage Items In-game](#5-leverage-items-in-game): Integrate collectibles in the game using the Sequence Indexer
6. [Burn In-game Achievement tokens](#6-burn-in-game-achievement-tokens): Burn game achievements with wagmi
7. [(Optional) Integrate Embedded Wallet Into Sequence Kit](#7-optional-integrate-embedded-wallet-into-sequence-kit): Enable smooth UX without the use of signer signed messages
...
You can get an Access Key and a Wallet Connect ID by doing the following:

https://docs.sequence.xyz/solutions/builder/getting-started/#step-3-optional-for-development-claim-an-api-access-key
...
Then, in order to send transactions to the blockchain in a seamless fashion that are gasless, implement a [Cloudflare Worker](/guides/templates/building-relaying-server) to mint items from a contract deployed in the previous step, ensuring that the transactions API contract address is inputted as a `Minter Role`
...
> [!NOTE]
> In a real game, this minting of achievement token would happen based on some trigger event in the game, for simplicity we've included of button
