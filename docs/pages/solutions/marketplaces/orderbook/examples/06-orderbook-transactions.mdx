...
<Callout type="info">
  ...
  to accomplish this
  [here](/solutions/wallets/universal-wallet/examples/01-connect-wallet)
</Callout>
...
In order to accept an existing request, one would need to use one of the `get` requests endpoints (like [getTopOrders](/solutions/marketplaces/orderbook/examples/03-get-top-orders)) to acquire a `orderId` inputted into the `requestId` parameter to submit a transaction, with a prior transaction of approving the currency for the request.
