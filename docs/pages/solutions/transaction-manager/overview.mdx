...
If you have the following installed: git, nodejs / NPM, and docker with docker runnning, you can simply run the following steps:

<Callout type="info">
See the full [steps here](https://github.com/0xsequence-demos/tx-manager/tree/master?tab=readme-ov-file#3-how-to-run-with-docker) if certain tools are not installed.
</Callout>
::::steps
#### Clone Repository

```sh
git clone git@github.com:0xsequence-demos/tx-manager.git
```
...
And set values [following steps 2-4](/solutions/transaction-manager/overview#sequence-builder-project-management)
...
Or, on a server to run in the background:

```sh
npm start:docker:detached
```
