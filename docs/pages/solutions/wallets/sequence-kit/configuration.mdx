...
As a alternative to [Step 4](/solutions/wallets/sequence-kit/getting-started#create-embedded-wallet-connector) in the quickstart guide, when creating a `wagmi` `connectors` variable for Universal Wallet, EOA, and Wallet Connect 2.0 integration, import the `getDefaultConnectors` function from the `@0xsequence/kit` package, and include a Wallet Connect ID obtained from [here](https://cloud.walletconnect.com/app), a default chain ID, app name, and the `projectAccessKey`.
