...
Simply, with a boolean passed in with `guest` key set to `true`, an ephemeral wallet will be created for use in the application with all other normal WaaS functions possible.

```typescript [App.tsx]
await sequence.signIn({ guest: true }, "Session name");
```
<Callout type="danger">
After the browser cache has been cleared, you will have a new guest wallet generated for the user. Therefore, assets stored in the guest wallet after a browser cache clear will not be accessible by the user.
</Callout>
