...
![data-flow-architecture](/img/diagrams/waas/standalone-overview.drawio.svg)
:::note
...
#### 3. Backend Forwards the Intent to the Partner Signing Service (PSS)

The partner signing service housed in the Nitro Enclave asks the WaaS API to build a transaction for the given intent and then validates that the transaction does not deviate from the original intent.
